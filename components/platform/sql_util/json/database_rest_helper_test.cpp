  ...
  "tables": [
    ...
    TEST(DatabaseRestHelperTest, UpdateTableValueEmptyStringBecomesNull)
    {
        ...
        testDb.RunInTransaction("UpdateTableValueEmptyStringBecomesNull",
            ...
            // Insert a person with a non-null value in the nullable
            // locked_until column. The empty-string-becomes-NULL behavior
            // under test is table-agnostic; people.locked_until is a
            // framework-owned nullable BIGINT, so this stays inside
            // honuware_platform with no app-table dependency.
            KeyValueTable insertRow = {
                {std::string(DbSchema::kPeopleEmail), "lockout@example.com"},
                {std::string(DbSchema::kPeopleFirstName), "Locked"},
                {std::string(DbSchema::kPeopleLastName), "Out"},
                {std::string(DbSchema::kPeoplePasswordHash), "hash"},
                {std::string(DbSchema::kPeopleLockedUntil), "2000000"},
            };
            int64_t personId = DbCrud::AddRowToTableFetchInt64PrimaryKey(
                transaction, databaseHelper,
                DbSchema::kPeopleTable, insertRow);
            std::string personIdStr = std::to_string(personId);

            // Verify initial locked_until is set
            KeyValueTable initialRow = DbCrud::LookupRowByValue(
                transaction, databaseHelper,
                DbSchema::kPeopleTable,
                DbSchema::kPeopleId,
                personIdStr);
            EXPECT_EQ(initialRow.at(std::string(DbSchema::kPeopleLockedUntil)), "2000000");

            // Update with empty string for locked_until — should become NULL
            Json::Value updateJson(Json::JsonObject{
                {std::string(DbSchema::kPeopleFirstName), "Updated"},
            ...
            // Verify locked_until is now NULL (read back as empty string by
            ...
            KeyValueTable updatedRow = DbCrud::LookupRowByValue(
                ...
                DbSchema::kPeopleId,
                personIdStr);
            EXPECT_EQ(updatedRow.at(std::string(DbSchema::kPeopleFirstName)), "Updated");
            EXPECT_EQ(updatedRow.at(std::string(DbSchema::kPeopleLockedUntil)), "");
        });
    }
