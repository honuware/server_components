...
void HandlePost(
    ...
    try {
        ...
        Json::Value result = PostUploadPhoto(
            endpointAuthHelper, req, resp, tableName, tableItemId, imageType);
        if (resp.code == 200) {
...
public:
    void AddRoute(WebApp* webApp) override {
        CROW_ROUTE(webApp->GetApp(), "/api/upload_photo/<string>/<int>/<string>")
            .methods(crow::HTTPMethod::POST)(
                [=](const crow::request& req, crow::response& resp,
                    const std::string& tableName, int64_t tableItemId,
                    const std::string& imageType) {
                    HandlePost(webApp, req, resp, tableName, tableItemId,
                        imageType);
                });
    }
} g_setupRouting;
...
Json::Value PostUploadPhoto(
    ...
    tp->RunInTransaction([&](Transaction& transaction) {
        ...
        auto uploadResult = imageHelper.UploadAndAssociatePhoto(
            transaction, tableName, tableItemId, imageBytes, imageType);
